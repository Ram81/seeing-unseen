...
from seeing_unseen.third_party.Detic.detic.config import (
    add_detic_config,
)  # noqa: E402
from seeing_unseen.third_party.Detic.detic.modeling.text.text_encoder import (
    build_text_encoder,
)  # noqa: E402
from seeing_unseen.third_party.Detic.detic.modeling.utils import (
    reset_cls_test,
)  # noqa: E402
...
class DeticPerception:
    def __init__(
        ...
        assert vocabulary in ["custom"]
        self.custom_vocab = False

        self.metadata = MetadataCatalog.get("__unused")
        self.metadata.thing_classes = args.custom_vocabulary.replace(
            "_", " "
        ).split(",")
        ...
        print(
            "Thing classes: {}".format(
                args.custom_vocabulary.replace("_", " ").split(",")
            )
        )
    ...
    def reset_vocab(self, new_vocab: List[str], vocab_type="custom"):
        ...
        if vocab_type == "custom":
            ...
            classifier = get_clip_embeddings(self.metadata.thing_classes)
            self.categories_mapping = {
                i: i + 1 for i in range(len(self.metadata.thing_classes))
            }
        else:
    ...
    def predict(
        ...
        """
        Arguments:
            obs: image of shape (H, W, 3) (in RGB order - Detic expects BGR)
        ...
        semantic_map, instance_map = overlay_masks(
            masks, class_idcs, (height, width), self.custom_vocab
        )
